...
Lib.CommandPoll {
  ...
  // Altera apenas o estado de energia (Botão Esquerdo)
  function toggleBt() {
  ...
  // --- SURFACE PROFILE ---
  property string localPerfState: "performance"

  Lib.CommandPoll {
    id: perfPoll
    running: root.active && root.visible
    interval: 5000
    command: sh("powerprofilesctl get 2>/dev/null || echo 'balanced'")
    parse: function(o) {
...
function cyclePerf() {
    ...
    var next = "balanced"
    // Lógica de ciclo para power-profiles-daemon (Padrão Arch)
    if (current === "power-saver") next = "balanced"
    else if (current === "balanced") next = "performance"
    else next = "power-saver"
    
    root.localPerfState = next
    det("powerprofilesctl set " + next)
  }
  ...
  ColumnLayout {
    ...
    RowLayout {
      ...
      Lib.ExpressiveButton {
        ...
        onRightClicked: {
            root.closeRequested()
            det("quickshell -p ~/.config/quickshell/snes-hub/lib/BluetoothMenu.qml")
        }
        fixX: -5
      }
