...
PanelWindow {
    ...
    // Only map when something is active
    ...
    // Background click-to-close area (placed BEFORE the content Item so it sits behind it in Z-order)
    MouseArea {
        anchors.fill: parent
    ...
    Item {
        x: masterWindow.currentX
        y: masterWindow.currentY
        ...
        Item {
            ...
            StackView {
                ...
                Keys.onEscapePressed: {
                    Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/quickshell/v2/components/ilyamiro/qs_manager.sh", "close"])
                    event.accepted = true
                }
