...
BarWidget {
  ...
  function cycleLayout() {
    ...
    // fcitx5's virtual keyboard is what actually types on this machine, but it's
    // excluded from typedKeyboards() (UNTYPED_KEYBOARDS matches hl-virtual-keyboard*)
    // and only exists intermittently, so it can never be targeted by name. Switch
    // every device together or the widget's label advances while real keystrokes
    // keep following the old layout.
