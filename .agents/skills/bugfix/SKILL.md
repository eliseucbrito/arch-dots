---
name: bugfix
description: "Bug hunter. Reproduces bugs with failing tests (RED), then fixes with TDD. Accepts a prompt, issue URL (GitHub or GitLab), or bug description. Simpler than /dev, focused solely on fixing bugs. Use when: bugfix, fix bug, debug, broken, regression, failing, doesn't work, something's wrong, fix this."
---
...
- `/bugfix <url>` — fix from a GitHub, GitLab, or Linear issue
...
**URL:** Fetch the issue. Pick the tool by the host in the URL — don't assume GitHub:

- **GitHub** (`github.com/<owner>/<repo>/issues/<number>`):
  `gh issue view <number> --json title,body --jq '.title + "\n\n" + .body'`
- **GitLab** (`gitlab.com/...` or any self-hosted GitLab instance — `git.<company>`, `gitlab.<company>`, etc.):
  `glab issue view <number> -F json | jq -r '.title + "\n\n" + .description'`
  - `glab` infers the repo from the current directory's git remote. If you're not inside the right repo, pass `-R <group>/<project>` (or the full host path for self-hosted).
  - Note GitLab calls the field `description`, not `body` — that's why the `jq` filter differs from GitHub's.
- **Linear** (`linear.app/...` or a `<TEAM>-<number>` identifier like `CORE-42`): `lineark issues read <identifier>` if `lineark` is available; otherwise ask the user to paste the issue text.

When the host is ambiguous (a bare number, or a self-hosted domain you don't recognize), ask which forge it is rather than guessing — running the wrong CLI just fails noisily and wastes a round-trip.
